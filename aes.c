...
void AESDecrypt(uint8_t ciphertext[DATA_SIZE], uint8_t plaintext[DATA_SIZE], uint8_t key[DATA_SIZE])
{
    // Convert key to matrix
    uint8_t master_key[STATE_ROW_SIZE][STATE_COL_SIZE];
    MessageToState(master_key, key);
    ...
    // Intermediate 9 rounds (1 - 9 included)
    for (targeted_round = 9; targeted_round >= 1; --targeted_round)
    {
        InvShiftRows(state);
        InvSubBytes(state);
...
void InvSubBytes(uint8_t state[STATE_ROW_SIZE][STATE_COL_SIZE])
{
    for (uint8_t i = 0; i < STATE_COL_SIZE; ++i)
        for (uint8_t j = 0; j < STATE_ROW_SIZE; ++j)
            state[i][j] = invsboxtab[state[i][j]];
}
...
void MCMatrixColumnProduct(uint8_t colonne[STATE_COL_SIZE])
{
    ...
    colonne[0] = GMul(0x02, col_copy[0]) ^ GMul(0x03, col_copy[1]) ^ col_copy[2] ^ col_copy[3];
    colonne[1] = col_copy[0] ^ GMul(0x02, col_copy[1]) ^ GMul(0x03, col_copy[2]) ^ col_copy[3];
    colonne[2] = col_copy[0] ^ col_copy[1] ^ GMul(0x02, col_copy[2]) ^ GMul(0x03, col_copy[3]);
    colonne[3] = GMul(0x03, col_copy[0]) ^ col_copy[1] ^ col_copy[2] ^ GMul(0x02, col_copy[3]);
}
...
void IMCMatrixColumnProduct(uint8_t colonne[STATE_COL_SIZE])
{
    ...
    colonne[0] = GMul(0x0e, col_copy[0]) ^ GMul(0x0b, col_copy[1]) ^ GMul(0x0d, col_copy[2]) ^ GMul(0x09, col_copy[3]);
    colonne[1] = GMul(0x09, col_copy[0]) ^ GMul(0x0e, col_copy[1]) ^ GMul(0x0b, col_copy[2]) ^ GMul(0x0d, col_copy[3]);
    colonne[2] = GMul(0x0d, col_copy[0]) ^ GMul(0x09, col_copy[1]) ^ GMul(0x0e, col_copy[2]) ^ GMul(0x0b, col_copy[3]);
    colonne[3] = GMul(0x0b, col_copy[0]) ^ GMul(0x0d, col_copy[1]) ^ GMul(0x09, col_copy[2]) ^ GMul(0x0e, col_copy[3]);
}
